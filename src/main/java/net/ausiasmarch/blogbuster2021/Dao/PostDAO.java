...
public class PostDAO {
    ...
    public ArrayList<PostBean> getPage(int page, int rpp, String param, String direction, String filtro) throws SQLException {
        ...
        parametro.add("cuerpo");
        parametro.add("fecha");
        parametro.add("etiquetas");
        
        ArrayList<String> direccion = new ArrayList<>();
        direccion.add("ASC");
        direccion.add("DESC");
        
        boolean existe_param = parametro.contains(param);
        boolean existe_direction = direccion.contains(direction);
        
        if (existe_param && existe_direction){
        oPreparedStatement = oConnection.prepareStatement("SELECT * FROM post WHERE "
                + "id LIKE ? OR titulo LIKE ? OR cuerpo LIKE ? OR fecha LIKE ? OR etiquetas LIKE ? ORDER BY " + param + " " + direction +" LIMIT ? OFFSET ?");
        oPreparedStatement.setString(1, "%"+filtro+"%");
        oPreparedStatement.setString(2, "%"+filtro+"%");
        oPreparedStatement.setString(3, "%"+filtro+"%");
        oPreparedStatement.setString(4, "%"+filtro+"%");
        oPreparedStatement.setString(5, "%"+filtro+"%");
        
        oPreparedStatement.setInt(6, rpp);
        oPreparedStatement.setInt(7, offset);
        ...
        return oPostBeanList;
            ...
            throw new InternalServerErrorException("Error en parámetros");
        }
    }
